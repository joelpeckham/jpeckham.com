...
- Finally, it's actually around 480 lines total, but since most of that is defining primitives which could be done in runtime, it's not _really_ relevant. It's just nice to have some starting points to work from :)
...
(For brevity a few interpret-time twins of the branching words are omitted here; the complete, runnable set lives [on GitHub](https://github.com/joelpeckham/pyforth).)
