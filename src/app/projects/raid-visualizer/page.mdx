...
- **Write data.** The array fills automatically when the page loads and when you change the RAID level or drive count. Edit the string (spaces become underscores) or hit **Random**, then press **Write →** only if you want to overwrite. Parity blocks appear in yellow as two-digit hex bytes.
...
- **Rebuild the array.** After a drive fails, its **Fail** button becomes **Fix**. Source blocks light up blue, then the recovered block turns red. Parity levels XOR surviving blocks. Mirrored levels copy from the paired drive.
- **Scale the array.** Add or remove drives within the limits of each RAID level. Usable capacity and fault tolerance update live, and the current text is rewritten across the new layout.
