...
Article [03](/posts/mysql-indexes/) left you with a solid composite. Then the handler does something like this:
...
This is post four of [Learn MySQL](/projects/mysql/). [Types](/posts/mysql-schema-types/) shaped the columns. The [primary key](/posts/mysql-primary-keys/) clustered the table. [Composites](/posts/mysql-indexes/) gave list endpoints a path. Today is about your `WHERE` and select list. Do they let that path get used?

## Filter × project

Every `SELECT` answers two questions: which rows survive, and which columns leave the server?<Cite n={1} /><Cite n={2} /> Apps feel both. Filter work is pages read while finding matches. Projection work is bytes copied, shipped over the network, and parked in the buffer pool. You hauled a wide row for a skinny card UI.

InnoDB still finds rows the way articles 02 and 03 described. Clustered descent for `WHERE id = ?`. Secondary descent plus bounce for org/status/date filters.<Cite n={3} /> This article is about two traps. Do not disable that machinery with a friendly-looking function. Do not ask for the novel when you needed the headline.
...
`SELECT *` is fine in a scratch session. In a list handler it couples your JSON to every future column. It also hauls `TEXT` / `JSON` / secrets you never render.<Cite n={6} /><Cite n={7} /> Wider rows mean fewer rows per InnoDB page. So the same filter touches more I/O even when the index match is perfect.
...
If the select list ever fits entirely inside a secondary index, MySQL may answer from the index without bouncing to the clustered row. That is a covering index.<Cite n={3} /><Cite n={8} /> Designing for it gets a whole article later in the series. Today, listing columns keeps that door open. It also stops you shipping kilobytes of `body` to an inbox table.
...
- Composite leftmost prefix matches the filters you send (article 03)
