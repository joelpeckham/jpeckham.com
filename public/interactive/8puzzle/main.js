...
function createPuzzleElem(puzzle) {
  ...
  for (let i = 0; i < puzzle.length; i++) {
    ...
    puzzlesquare.addEventListener(
      transitionEnd,
      () => {
        puzzlesquare.classList.add("doneTransition");
      },
      false,
    );
    if (puzzle[i] == "-") {
...
async function solve() {
  ...
  document.querySelector("#stats p").innerText =
    `Stats: "${puzStrVal}" solved in ${timeDelta} seconds. Max depth: ${maxDepth}. Total visited: ${totalVisited}. Solution length: ${solution.length}`;
...
function animateTranslations(solution = lastSolution, start, increment) {
  ...
  let movement_vector_y =
    Math.floor(next.indexOf("-") / 3) - Math.floor(current.indexOf("-") / 3);
